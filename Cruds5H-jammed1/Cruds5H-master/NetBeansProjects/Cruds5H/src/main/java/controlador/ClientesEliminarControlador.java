...
public class ClientesEliminarControlador implements ActionListener {

    private final ClientesEliminar view;
    private final Connection con;

    private JButton btnListo;     
    private JButton btnRegresar;  

    public ClientesEliminarControlador(ClientesEliminar view, Connection con) {
        this.view = view;
        this.con  = con;
    ...
    private void hookPrivateButtons() {
        try {
            Field fListo = ClientesEliminar.class.getDeclaredField("jButton1");
            fListo.setAccessible(true);
            btnListo = (JButton) fListo.get(view);
            btnListo.addActionListener(this);

            Field fRegresar = ClientesEliminar.class.getDeclaredField("jButton2");
    ...
    private boolean camposCoincidenConBD(int id) {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                return vNombre.equals(dbNombre) && vCorreo.equals(dbCorreo) && vTel.equals(dbTel);
            }
        } catch (SQLException ex) {
